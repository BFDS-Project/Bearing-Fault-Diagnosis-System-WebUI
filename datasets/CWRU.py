...
def data_load(filename, axisname, label):
    ...
    while end <= fl.shape[0]:
        data.append(fl[start:end]) # 将长串的数据分成signal_size的长度的很多份
        lab.append(label)
        start += signal_size
        end += signal_size
...
class CWRU:
    ...
    def __init__(self, data_dir, transfer_task, normlizetype="0-1"):
        ...
        self.data_transforms = {
            'train': Compose([
                Reshape(), # 转置
                Normalize(self.normlizetype), # 归一化
                # RandomAddGaussian(),
                # RandomScale(),
                # RandomStretch(),
                # RandomCrop(),
    ...
    def data_split(self, transfer_learning=True):
        if transfer_learning:
            ...
            source_val = dataset(list_data=val_pd, transform=self.data_transforms['val']) # 返回的是实例化后的数据集类
